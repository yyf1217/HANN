...
mkdir -p logs_add_5d_l
TASK_FILE="logs_add_5d_l/all_tasks.txt"
: > "$TASK_FILE"
...
for task in $HOM; do
    ...
    echo "python main_count_add_5d.py --model L --task $task_args --seed $seed" >> "$TASK_FILE"
    echo "python main_count_add_5d.py --model LF --task $task_args --seed $seed" >> "$TASK_FILE"
done
...
for task in $ISO; do
    for level in $LEVEL; do
        ...
        echo "python main_count_add_5d.py --model L --task $task $level --seed $seed" >> "$TASK_FILE"
        echo "python main_count_add_5d.py --model LF --task $task $level --seed $seed" >> "$TASK_FILE"
    done
done
...
for idx in $(seq 0 $((NGPU-1))); do
    ...
    " > "logs_add_5d_l/gpu_${gpu}.log" 2>&1 &
done
