...
class NodeEmbedding(nn.Module):
    def __init__(self, dim: int, max_dis: int, max_h: int, enc: Optional[nn.Module]):
        super().__init__()
        self.max_dis = max_dis
        self.max_h = max_h
        self.embed_v = enc and enc(dim)
        self.embed_d = nn.Embedding(max_dis + 1, dim)
        # self.embed_h = nn.Embedding(max_h + 1, dim)
        self.embed_h = nn.Sequential(
            nn.Linear(5, int(dim/2)),
            nn.ReLU(),
            nn.Linear(int(dim/2), dim)
        )
        self.embed_hl = nn.Sequential(
            nn.Linear(1, int(dim/2)),
            nn.ReLU(),
            nn.Linear(int(dim/2), dim)
        )
        # self.embed_h = MLP(5, dim, norm=True)
        self.alpha = nn.Parameter(torch.tensor(0.0))
        self.fuse_dh = nn.Sequential(
            nn.Linear(2 * dim, dim),
            nn.ReLU(),
            nn.Linear(dim, dim)
        )
    ...
    def forward(self, batch):
        ...
        # h = self.embed_h(torch.clamp(batch.h, 0, max=self.max_h))
        h = self.embed_h(batch.h)
        hl = self.embed_hl(batch.hl)
        alpha = torch.sigmoid(self.alpha)   # ∈ (0,1)
        h_all = h + alpha * hl
        # dh_emb = self.fuse_dh(torch.cat([d, h, hl], dim=-1))   # [N, dim]
        dh_emb = self.fuse_dh(torch.cat([d, h_all], dim=-1))   # [N, dim]
        batch.x = x + d + dh_emb
        del batch.d
        return batch
...
class GNN(nn.Sequential):
    ...
    def __init__(self, args, task: str, enc_a: Optional[nn.Module], enc_e: Optional[nn.Module], odim: int, bn: bool=True):
        ...
        self.add_module("in", NodeEmbedding(idim:=args.dim_embed, args.max_dis, args.max_h, enc_a))
