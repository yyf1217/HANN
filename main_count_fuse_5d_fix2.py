import torch
from src.utils_add_fix2 import *
from src.model_fuse_5d_fix2 import *
...
parser.add_argument("--outdir", type=str, default="result_count_fuse_5d_fix3", help="output")
parser.add_argument("--device", type=int, default=0, help="CUDA device")

parser.add_argument("--max_dis", type=int, default=5, help="distance encoding")
parser.add_argument("--max_h", type=int, default=5, help="homotopy encoding")
...
# dataloader = {
#     name: DataLoader(
#         dataset_add.GraphCount(args.indir,
#                            name, *args.task,
#                            lifted=(args.model in ["Sub", "L", "LF"])
#         ),
#         follow_batch=["y"],
#         batch_size=args.bs,
#         num_workers=4,
...
pt_dir = 'data/count/processed_5d_fix2/{}_{}.pt'.format(args.task[0], args.task[1])
if os.path.exists(pt_dir):
    train_ds, val_ds, test_ds = torch.load(pt_dir, weights_only=False)
else:
    train_raw = dataset.GraphCount(args.indir, 'train', *args.task)
    val_raw = dataset.GraphCount(args.indir, 'val', *args.task)
    test_raw = dataset.GraphCount(args.indir, 'test', *args.task)
    
    train_ds = [subgraph(train_raw[i]) for i in range(len(train_raw))]
    val_ds = [subgraph(val_raw[i]) for i in range(len(val_raw))]
    test_ds = [subgraph(test_raw[i]) for i in range(len(test_raw))]
...
dataloader = {
    'train': DataLoader(train_ds, follow_batch=['y'], batch_size=args.bs, shuffle=True),
    'val': DataLoader(val_ds, follow_batch=['y'], batch_size=args.bs, shuffle=False),
    'test': DataLoader(test_ds, follow_batch=['y'], batch_size=args.bs, shuffle=False),
}
...
optim = torch.optim.Adam([
    {"params": getattr(model, "in").embed_h.parameters(), "lr": 5*args.lr},
    {"params": [p for n, p in model.named_parameters() if not n.startswith("in.embed_h")], "lr": args.lr},
])
